...
describe 'Events Index:', type: :request do
  describe 'When I visit /api/v1/events' do
    ...
    it 'I get a JSON response with all events in the database' do
      ...
      expect(response[:events][0][:events].include?("Archery Men's Individual")).to be(true)
      expect(response[:events][0][:events].include?("Archery Men's Team")).to be(true)
      expect(response[:events][0][:events].include?("Archery Women's Individual")).to be(true)
      expect(response[:events][0][:events].include?("Archery Women's Team")).to be(true)

      expect(response[:events][1]).to have_key(:sport)
      expect(response[:events][1][:sport]).to eq('Badminton')
      ...
      expect(response[:events][1][:events].include?("Badminton Men's Doubles")).to be(true)
      expect(response[:events][1][:events].include?("Badminton Men's Singles")).to be(true)
      expect(response[:events][1][:events].include?("Badminton Women's Doubles")).to be(true)
      expect(response[:events][1][:events].include?("Badminton Women's Singles")).to be(true)
      expect(response[:events][1][:events].include?("Badminton Mixed Doubles")).to be(true)
    end
  end
end